...
| `ClearState` | `3` | _Do not_ execute the Approval Program, and instead execute the Clear State Program (which may not reject this transaction). Additionally, clear any _local state_ for the _application ID_ out of the _sender_'s account data (as in `CloseOut`). |
...
| `DeleteApplication` | `5` | _After_ executing the Approval Program, _delete_ the parameters with the _application ID_ from the account data of the application's creator. |
