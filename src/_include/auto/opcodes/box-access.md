| OPCODE | DESCRIPTION |
| :-: | :---------- |
| `box_create` | create a box named A, of length B. Fail if the name A is empty or longer than 64 bytes, or B exceeds 32,768. Returns 0 if A already existed, else 1 |
...
| `box_resize` | change the size of box named A to be of length B, adding zero bytes to end or removing bytes from the end, as needed. Fail if the name A is empty or longer than 64 bytes, A is not an existing box, or B exceeds 32,768. |
| `app_box_create` | create a box named B, of length C, for app A. Fail if the name B is empty or longer than 64 bytes, or C exceeds 32,768. Returns 0 if B already existed, else 1 |
...
| `app_box_resize` | change the size of box named B of app A to be of length C, adding zero bytes to end or removing bytes from the end, as needed. Fail if the name B is empty or longer than 64 bytes, B is not an existing box, or C exceeds 32,768. |
